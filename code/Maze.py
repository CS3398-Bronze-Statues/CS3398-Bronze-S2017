...
class Maze(object):
    ...
    def update(self):
        if self.player.x == 0 and self.player.y == 0:
            ...
            self.east = True
            self.west = False
            self.output = "You are in a small room with slimy stone walls to the\nwest and the south. To your north " +\
                "and east you see dark\npassages."
        ...
        (self.player.x == 2 and self.player.y == -1) or (self.player.x == 3 and self.player.y == -2)):
            ...
            self.output = "To your west you see the darkness of the passageway. To your east, you\nsee a faint red " +\
                "glow outlining what appears to be a door..."
            ...
            if self.player.weapon == True:
                self.output = "You hurl open the door and crash into the room. A hideous scream\nechos through " +\
                    "the stone chamber in which you find yourself.\nImmediately, a massive, fiery, bat-like " +\
                    "creature launches itself at you from the\nother side of the room. You have woken Beelzebub, " +\
                    "the demon of death! After a few minutes of fierce combat, you have emerged victorious with " +\
                    "Beelzebub's corpse laying at your feet. After a moment to recover and catch your breath you " +\
                    "focus your gaze upon the door with daylight glowing through it more than ever before."

            else:
                self.output = "You cautiously open the door a crack and hear a giant snore.\nThrough the crack you " +\
                    "can see red glow is caused by a giant\nmonster sleeping on the floor. Behind his massive bulk " +\
                    "you see a stone\ndoor with daylight peeking through the keyhole! To escape you'll have to " +\
                    "\nkill this beast...but you'll need a weapon.\nKarate just isn't going to be enough."
            ...
            self.output = "To the south is only dark passageway. To the north you see\nthe end of the passageway " +\
                "and the silhouette of a large object\non the ground."

        elif self.player.x == 4 and self.player.y == -3:
            self.north = False
            self.south = False
            self.west = True
            self.east = False
            self.output = "You have made it to freedom! Your people will tell stories about you for ages to come.\n" +\
                "You are a now a living legend. Congratulations!"
            ...
            self.output = "You approach the object and find that it is a massive obsidian chest...\ncovered in " +\
                    "cobras! Fortunately, you are an accomplished cobra whisperer,\nso you sternly tell the snakes " +\
                    "to move and proceed to open the chest.\nInside you find nothing. The chest has no bottom and " +\
                    "drops into\na black void. Again using your fortunate cobra powers, you command the\n snakes to " +\
                    "tie themselves into a rope which you then use to\nlower yourself into the void. Here you find " +\
                    "an obsidian sword\n floating in the air - defying the laws of physics. Seizing it, you " +\
                    "climb\nback up, close the chest, and tearfully kiss your faithful cobras goodbye.\nYou are " +\
